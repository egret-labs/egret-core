...
var CodeUtil = require("./code_util.js");
/**
 * 键是类名，值为这个类依赖的类名列表
 */
var classInfoList;
...
module globals {
    export function joinEgretDir(current: string, arg0) {

    }

    export function log(code: number) {
        console.log(code);
    }

    export function exit(code, ...params: string[]) {
 ...
 * 创建manifest列表
 */
export function create(srcPath, createAll: boolean, referenceInfo?, hasSwan1?, fileList?: string[]) {
    hasSwan = !!hasSwan1;

    srcPath = escapeSrcPath(srcPath);
    var manifest = getManifest(srcPath, fileList);
    if (referenceInfo) {
 ...
 */
function getManifest(srcPath, fileList?: string[]) {
    resetCache();
    var manifest = fileList || file.searchByFunction(srcPath, filterFunc);
    var exmlList = [];
...
function readReferenceFromExml(path) {
    ...
    var text = file.read(path);
    var exml = undefined // xml.parse(text);
    if (!exml) {
        return;
    }
    var list = [];
