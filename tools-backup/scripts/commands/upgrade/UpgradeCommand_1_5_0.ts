...
class UpgradeCommand_1_5_0 implements egret.Command {
    ...
    private upgradeTo_1_5_0() {
        ...
        //替换 native_loader.js
        var loaderPath = file.join(projectDir, "launcher", "native_loader.js");
        var loaderContent = file.read(loaderPath);
        //保存副本
        file.save(file.join(projectDir, "launcher", "copy_native_loader.js"), loaderContent);


        //生成egret_loader.js样板
        var fileContent = file.read(file.join(params.getEgretRoot(), "tools", "templates", "empty", "launcher", "runtime_loader.js"));
        file.save(file.join(projectDir, "launcher", "runtime_loader.js"), fileContent);
        var fileContent = file.read(file.join(params.getEgretRoot(), "tools", "templates", "empty", "launcher", "native_loader.js"));
        file.save(file.join(projectDir, "launcher", "native_loader.js"), fileContent);
        var fileContent = file.read(file.join(params.getEgretRoot(), "tools", "templates", "empty", "launcher", "native_require.js"));
        file.save(file.join(projectDir, "launcher", "native_require.js"), fileContent);
