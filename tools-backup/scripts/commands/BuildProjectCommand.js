...
var GetProjectFileListCommand = require("./GetProjectFileListCommand");
/**
 * 执行一次tsc
 */
var BuildProjectCommand = (function () {
    function BuildProjectCommand() {
    }
    BuildProjectCommand.prototype.getModuleReferenceList = function () {
        return this.moduleReferenceList;
    };
    BuildProjectCommand.prototype.execute = function (callback) {
        var _this = this;
        var time = Date.now();
        ...
        moduleNames.forEach(function (moduleName) {
            if (config.getModuleDecouple(moduleName)) {
                ...
                var array = [];
                var list = file_list.filter(function (item) {
                    item = file.join(prefix, source, item);
                    array.push(item);
                    if (item.indexOf(".js") >= 0) {
                        return true;
                    }
                    if (!_this.moduleReferenceList || _this.moduleReferenceList.indexOf(item) >= 0) {
                        return true;
                    }
                    return false;
                });
    ...
    BuildProjectCommand.prototype.generateExmlDTS = function (srcPath) {
